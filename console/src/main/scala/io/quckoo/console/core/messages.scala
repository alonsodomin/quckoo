...
final case class Login(username: String,
    ...
    extends Command
final case class LoggedIn(passport: Passport, referral: Option[ConsoleRoute])
    extends Event
...
final case class RefreshJobSpecs(
    ...
    result: Try[Map[JobId, Pot[JobSpec]]] = Failure(
      new AsyncAction.PendingException)
) extends AsyncAction[Map[JobId, Pot[JobSpec]], RefreshJobSpecs] {

  override def next(newState: PotState,
                    newValue: Try[Map[JobId, Pot[JobSpec]]]): RefreshJobSpecs =
    copy(state = newState, result = newValue)

}

final case class RegisterJobResult(jobId: ValidatedNel[QuckooError, JobId])
    extends Event
...
final case class RefreshExecutionPlans(
    keys: Set[PlanId],
    state: PotState = PotState.PotEmpty,
    result: Try[Map[PlanId, Pot[ExecutionPlan]]] = Failure(
      new AsyncAction.PendingException)
...
final case class ExecutionsLoaded(tasks: Map[TaskId, Pot[TaskExecution]])
    extends Event

final case class RefreshExecutions(
    keys: Set[TaskId],
    state: PotState = PotState.PotEmpty,
    result: Try[Map[TaskId, Pot[TaskExecution]]] = Failure(
      new AsyncAction.PendingException)
