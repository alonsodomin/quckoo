...
/**
  ...
  */
trait ConsoleOps2 { this: LoggerHolder =>

  def refreshClusterStatus: ConsoleIO[ClusterStateLoaded] =
    for {
      _            <- ConsoleIO.local(IO(logger.debug("Refreshing cluster status...")))
      clusterState <- ConsoleRemote.currentState.map(ClusterStateLoaded)
    } yield clusterState

  def registerJob(jobSpec: JobSpec): ConsoleIO[RegisterJobResult] =
    ConsoleRemote.registerJob(jobSpec).map(RegisterJobResult)
