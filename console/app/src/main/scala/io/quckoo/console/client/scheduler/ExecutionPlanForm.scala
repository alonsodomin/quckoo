...
import io.quckoo.console.client.core.{ConsoleScope, LoadJobSpecs}
...
object ExecutionPlanForm {
  ...
  case class Props(proxy: ModelProxy[ConsoleScope], plan: Option[ExecutionPlan], handler: ScheduleHandler)
  ...
  def apply(proxy: ModelProxy[ConsoleScope], schedule: Option[ExecutionPlan], handler: ScheduleHandler) =
