...
object Patterns {
  ...
  private def resolvePattern(base: String, pattern: String): String = {
    ...
    if (normalized.endsWith("/") || pattern.startsWith("/")) normalized + pattern
...
final class Patterns private (val ivyPatterns: Seq[String],
  ...
  def withArtifacts(patterns: String*): Patterns =
    Patterns(ivyPatterns, patterns ++ artifactPatterns, mavenCompatible)
  def withIvys(patterns: String*): Patterns =
    Patterns(ivyPatterns ++ patterns, artifactPatterns, mavenCompatible)

  override def toString: String =
    s"Patterns(ivyPatterns = $ivyPatterns, artifactPatterns = $artifactPatterns, mavenCompatible = $mavenCompatible)"
