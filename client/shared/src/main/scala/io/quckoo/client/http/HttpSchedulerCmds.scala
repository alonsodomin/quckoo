...
trait HttpSchedulerCmds extends HttpMarshalling with SchedulerCmds[HttpProtocol] {
  ...
  implicit lazy val scheduleJobCmd: ScheduleJobCmd =
    new Auth[HttpProtocol, ScheduleJob, Either[JobNotFound, ExecutionPlanStarted]] {
      override val marshall =
        marshallToJson[ScheduleJobCmd](HttpMethod1.Put, _ => ExecutionPlansURI)
  ...
  implicit lazy val getPlansCmd: GetPlansCmd =
    new Auth[HttpProtocol, Unit, Seq[(PlanId, ExecutionPlan)]] {
      override val marshall   = marshallEmpty[GetPlansCmd](HttpMethod1.Get, _ => ExecutionPlansURI)
      override val unmarshall = unmarshallFromJson[GetPlansCmd]
    }

  implicit lazy val getPlanCmd: GetPlanCmd =
    new Auth[HttpProtocol, PlanId, Option[ExecutionPlan]] {
      override val marshall   = marshallEmpty[GetPlanCmd](HttpMethod1.Get, planUrl)
  ...
  implicit lazy val getExecutionsCmd: GetExecutionsCmd =
    new Auth[HttpProtocol, Unit, Seq[(TaskId, TaskExecution)]] {
      override val marshall =
        marshallEmpty[GetExecutionsCmd](HttpMethod1.Get, _ => TaskExecutionsURI)
      override val unmarshall = unmarshallFromJson[GetExecutionsCmd]
    }

  implicit lazy val getExecutionCmd: GetExecutionCmd =
    new Auth[HttpProtocol, TaskId, Option[TaskExecution]] {
      override val marshall   = marshallEmpty[GetExecutionCmd](HttpMethod1.Get, executionUrl)
  ...
  implicit lazy val cancelPlanCmd: CancelPlanCmd =
    new Auth[HttpProtocol, PlanId, Either[ExecutionPlanNotFound, ExecutionPlanCancelled]] {
      override val marshall = marshallEmpty[CancelPlanCmd](HttpMethod1.Delete, planUrl)
      override val unmarshall =
