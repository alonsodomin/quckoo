...
class IvyResolver private[ivy] (ivy: Ivy) extends Resolver[IO] with LazyLogging {
  ...
  private[this] def resolve(artifactId: ArtifactId, download: Boolean): IO[Resolved[Artifact]] =
    IO {
      def unresolvedDependencies(report: ResolveReport): Resolved[ResolveReport] = {
        ...
        validations.foldLeft(report.validNel[DependencyError])(
          (acc, v) => (acc, v.toValidatedNel).mapN((_, r) => r)
        )
      }
