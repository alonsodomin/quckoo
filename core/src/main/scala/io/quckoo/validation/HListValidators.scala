...
trait HListValidators {
  ...
  def fieldValidator[K <: Symbol, F[_]: Applicative, A](validator: ValidatorK[F, A])(
      implicit witness: Witness.Aux[K]): ValidatorK[F, FieldType[K, A]] = {
  ...
  implicit def hlistValidator[K <: Symbol, F[_]: Applicative, H, T <: HList](
      ...
      hValidator: Lazy[ValidatorK[F, FieldType[K, H]]],
      tValidator: ValidatorK[F, T]): ValidatorK[F, FieldType[K, H] :: T] = {
    (hValidator.value * tValidator).dimap(hlist => (hlist.head, hlist.tail), _.map {
      case (h, t) => h :: t
    })
  }
  ...
  implicit def genericValidator[F[_]: Applicative, A, R](
      ...
      validator: Lazy[ValidatorK[F, R]]): ValidatorK[F, A] = {
